...
**Project standards (required before writing artifacts):** OpenSpec injects `openspec/config.yaml` `context` + `rules` via `openspec instructions`. Also read the underlying docs that apply to the change:

| Doc | When |
|-----|------|
| `AGENTS.md` | Always |
| `docs/design_guideline.md` | Any UI / surface / look |
| `frontend/AGENTS.md` + `frontend/docs/frontend-standards.md` | Frontend (Screaming Architecture → `features/<capability>/`) |
| `backend/docs/backend-standards.md` | Backend (Clean Architecture layers) |

Proposal / design / tasks **must** respect those standards (concrete feature paths and CA layers in design + tasks). Do not invent Astryx components or HTTP shapes.
...
- Honor `openspec/config.yaml` context/rules and the project standards docs (Screaming Architecture front, Clean Architecture back, design guideline)
